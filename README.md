...
下图是一个完整区间的累计净能耗-位置曲线，作为在线预测的样本数据：

![完整区间能耗曲线](verify_prediction/plots/pos_energy_curve.png)

在此曲线上，以 **100ms 为步长**滑动时间窗口，模拟 Backend1 每 100ms 向 Backend2 发起一次预测请求：

```
窗口 [0,    800ms] ──→ Backend2 预测 [800,  1000ms]
窗口 [100,  900ms] ──→ Backend2 预测 [900,  1100ms]
窗口 [200, 1000ms] ──→ Backend2 预测 [1000, 1200ms]
...
```
